...
<script>
	$(document).ready(function() {
			...
			$('form').submit(function(){
				 ...
				 if(CKEDITOR.instances.rb_content.getData().length < 1){
						alert("본문입력 필수");
				        $('.rb_content').focus();
				         return false;
				 }
...
<body>
	...
	<div id="content_wrap">
	<form action="${conPath }/rbModify.do"  method="post" enctype="multipart/form-data">
		<input type="hidden" name="rb_no" value="${rb.rb_no }"> 	
		<table class="table is-striped is-narrow is-hoverable">
			<caption>${rb.rb_no }글수정</caption>
			<tbody>
				<tr>
					<th>글제목</th>
					<td><input type="text" name="rb_name" class="rb_name" value="${rb.rb_name }"></td>
				</tr>
				<tr id="write_area">
					<th>본문</th>
					<td>
						<textarea rows="5" cols="30" name="rb_content" class="rb_content">${fb.rb_content }</textarea>
					</td>
				</tr>
			</tbody>
		</table>
		<table id="table_bottom">
			<tr>
				<td>
					...
					<input type="button" value="목록" onclick="location.href='${conPath }/reviewListView.do?pageNum=${pageNum }'"
