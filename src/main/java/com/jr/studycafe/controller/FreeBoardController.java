...
import com.jr.studycafe.service.FreeCommentsService;
...
public class FreeBoardController {
	...
	@Autowired
	private FreeCommentsService fcService;
	...
	public String detail(FreeBoard b, Model model, HttpSession httpSession, String pageNum2,FreeComments f ) {
		if(hitable == 1) {
		...
		f.setFb_no(b.getFb_no()); 
		model.addAttribute("freeCommentsList", fcService.freeCommentsList(pageNum2, model, f));
	...
	//여기서부터 댓글부분
	@RequestMapping(value="freeCommentsWrite")
	public String freeCommentsWrite(Model model, FreeComments f, HttpSession httpSession) {
		fcService.freeCommentsWrite(f, httpSession, model);
		return "forward:freeBoardDetail.do";
	}
	@RequestMapping(value = "freeCommentsDelete")
	public String freeCommentsDelete(Model model, FreeComments f) {
		fcService.freeCommentsDelete(f.getC_no());
		return "forward:freeBoardDetail.do";
	}
	@RequestMapping(value="freeCommentsAppend", method=RequestMethod.GET)
	public String freeCommentsAppend(String pageNum2, Model model, FreeComments f) {
		model.addAttribute("appendList",fcService.freeCommentsList(pageNum2, model, f));
		return "freeBoard/freeComments_append";
	}
}
