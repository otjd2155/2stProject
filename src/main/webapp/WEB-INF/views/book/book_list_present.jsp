...
<body>
	...
	$(document).ready(function(){
		...
		<table class="table is-striped is-narrow is-hoverable">
			...
			<tbody>
				<c:if test="${not empty bookList_present }">
				<c:forEach items="${bookList_present}" var="book">		
						<tr>
							<td>${oNum }</td>
							<td>${book.roomname}</td>
							<td>${book.bk_date}</td>
							...
							<td>${book.bk_tp}</td>
							<td><div
									style="color: white; background-color: #ffb400; width: 20px; display: inline; font-size: 0.7em; border-radius: 2px; padding: 2px;">예약됨</div>
							</td>
						</tr>
						<c:set var="oNum" value="${oNum+1 }" />
					</c:forEach>
					</c:if>
				<c:if test="${empty bookList_present }">
						<tr>
						...
						<nav class="pagination" role="navigation" aria-label="pagination">
							<c:if test="${paging.startPage>paging.blockSize }">
								<a class="pagination-previous" title="This is the first page"
									href="${conPath }/bookListView_present.do?pageNum=${paging.startPage-1}">Previous</a>
							</c:if>
							<c:if test="${paging.endPage<paging.pageCnt }">
								<a class="pagination-next"
									href="${conPath }/bookListView_present.do?pageNum=${paging.endPage+1}">Next
									page</a>
							</c:if>
								...
								<c:forEach var="i" begin="${paging.startPage }"
									...
									<c:if test="${paging.currentPage!=i }">
										<a class="pagination-link" aria-label="Goto page 2"
											href="${conPath }/bookListView_present.do?pageNum=${i}">${i }</a>
									</c:if>
								</c:forEach>
							</ul>
						</nav>
