...
<script>
	$(document).ready(function() {
			...
			$('form').submit(function(){
				 ...
				 if(CKEDITOR.instances.rv_content.getData().length < 1){
						alert("본문입력 필수");
				        $('.rv_content').focus();
				         return false;
				 }
...
<body>
	...
	<form action="${conPath }/reviewWrite.do"  method="post" enctype="multipart/form-data">	
		<input type="hidden" name="bk_no" value="${param.bk_no }">
		<table class="table is-striped is-narrow is-hoverable">
			<caption>글쓰기</caption>
			<tbody>
				<tr>
					<th>글제목</th>
					<td><input type="text" name="rv_name" class="rv_name"></td>
				</tr>
				<tr id="write_area">
					<th>본문</th>
					<td>
						<textarea rows="5" cols="30" name="rv_content" class="rv_content"></textarea>
					</td>
				</tr>
			</tbody>
		</table>
