...
ALTER TABLE ReviewBoard 
    ADD (rv_date DATE DEFAULT SYSDATE); -- 방번호 추가
...
INSERT INTO ReviewBoard (rv_no, rv_name, rv_content, rv_status, rv_hit, u_Id, bk_no) 
    VALUES (RV_NO_SQ.NEXTVAL, '이런', '하나크123', 1, 0, 'ses2155', 21);
...
-- 리뷰 글 쓰기
INSERT INTO ReviewBoard (rv_no, rv_name, rv_content, rv_status, rv_hit, u_Id, bk_no) 
    VALUES (RV_NO_SQ.NEXTVAL, '제목3', '내용3', 1, 0, 'aaaa', 10);
...
SELECT R.*, U.U_NICKNAME FROM RvComments R, USERS U WHERE R.U_ID= U.U_ID AND C_STATUS=1 AND RV_NO = 25 ORDER BY C_RDATE DESC;
SELECT * FROM (SELECT ROWNUM RN, A.* FROM (SELECT R.*, U.U_NICKNAME FROM RvComments R, USERS U WHERE R.U_ID= U.U_ID AND C_STATUS=1 AND RV_NO = 25 ORDER BY C_RDATE DESC)A) WHERE RN BETWEEN 1 AND 20 ;
