			...
			<tbody>
					...
					<c:forEach items="${bookList_past}" var="book">			
						<tr>
							<td>${oNum }</td>
							<td>${book.roomname}</td>
							<td class="bk_date">${book.bk_date}</td>
							<td>${book.bk_stime}</td>
							<td>${book.bk_etime}</td>
							<td>${book.bk_tt}</td>
							<td>${book.bk_tp}</td>
							...
							<td>
								<c:if test="${book.bk_rv_status eq 0}">
									<input type="button" value="후기쓰기" onclick="location.href='${conPath }/reviewWriteView.do?bk_no=${book.bk_no }'" id="book_review">
								</c:if>		
								<c:if test="${book.bk_rv_status eq 1}">
									<span style="font-weight: bold; color: #ffb400;" >작성 완료</span> 
								</c:if>	
							</td>
						</tr>
